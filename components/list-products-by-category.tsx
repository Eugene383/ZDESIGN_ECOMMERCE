    ...
    <main className="w-full">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {productsByCategory.map((productbyCategory) => {
          const productImages = Array.isArray(productbyCategory.product_images)
            ? productbyCategory.product_images
            : productbyCategory.product_images
              ? [productbyCategory.product_images]
              : [];

          const imageUrl =
            productImages.find((image: any) => image.is_primary)?.image_url ??
